...
update: ## Update code without purging data
	...
	-$(COMPOSE) stop worker
	-sh -c 'sleep 30'
	-$(COMPOSE) exec -T -u www-data web sh -c 'TYPO3_SITE_BASE="https://$${TYPO3_DOMAIN:-localhost}/" vendor/bin/typo3 messenger:consume index nr_ai_search --limit=4 --time-limit=90 -vv 2>&1 | tail -30'
