.PHONY: up down reset update logs shell db-shell seed export-seed build clean help
...
export-seed: ## Export current DB as new seed
	$(COMPOSE) exec -T db mariadb-dump -u root -p"$${MARIADB_ROOT_PASSWORD:-root}" "$${MARIADB_DATABASE:-typo3}" \
		--single-transaction --quick --skip-lock-tables | gzip > data/db.sql.gz
...
db-shell: ## Open MariaDB shell
	$(COMPOSE) exec db mariadb -u typo3 -ptypo3 typo3
